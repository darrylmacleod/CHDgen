...
***Usage
Quickstart
Download chd-flow-diagram-builder.html
Open it in Chrome, Firefox, Edge, or Safari
A sample e-commerce CHD flow loads automatically as a reference
Click ⬜ New to clear the canvas and start your own diagram
Building a Diagram
Click a component type in the left sidebar to arm your cursor
Click on the canvas to place the component (click again to place another; press Esc to stop)
Hover a placed component to reveal its connection ports (small blue circles)
Drag from a port to another component to create a connection
With the connection selected, use the Properties panel to set the Flow Type and add a label
Double-click any component to edit its label; right-click for delete options
Tips
